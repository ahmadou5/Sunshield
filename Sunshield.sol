...
abstract contract SunshieldBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	string private constant SYMBOL = "SUNSHIELD";
	...
	uint256 private constant _totalTokens = 10000000000000000 * 10**DECIMALS;	//1 quadrillion total supply
	...
	constructor (address routerAddress) {
		...
		// Initialize PancakeSwap V2 router and sunshine <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
		setPancakeSwapRouter(routerAddress);

		// 4% liquidity fee, 12% reward fee, 6% additional sell fee
		setFees(4, 12, 6);
	...
	// This function swaps a {tokenAmount} of sunshield tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for sunshield -> WBNB
	...
	function swapBNBForTokens(uint256 bnbAmount, address to) internal returns(bool) { 
		// Generate pair for WBNB -> sunshield
		address[] memory path = new address[](2);
	...
	// Returns how many more $sunshield tokens are needed in the contract before triggering a swap
...
contract Sunshield is SunshieldBase {
	...
	// REWARD CYCLE
	uint256 private _rewardCyclePeriod = 4 hours; // The duration of the reward cycle (e.g. can claim rewards once a day)
	...
	uint256 private _totalBNBAsSunshieldClaimed; // The total number of BNB that was converted to sunshield and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsSunshieldClaimed; // The amount of BNB converted to sunshield and claimed by each address
	...
	bool private _reimburseAfterSunshieldClaimFailure; // If true, and sunshield reward claim portion fails, the portion will be given as BNB instead
	...
	function onActivated() internal override {
		...
		setReimburseAfterSunshieldClaimFailure(true);
		setMinRewardBalance(500000 * 10**decimals());  //At least 500000 tokens are required to be eligible for rewards
	...
	function onBeforeTransfer(address sender, address recipient, uint256 amount) internal override {
		...
		bool isSelling = isPancakeSwapPair(recipient);
		if (!isSelling) {
			// Wait for a dip, sunshield diamond hands
			return;
		}
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough sunshield balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim sunshield tokens
		if (!claimSunshield(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterSunshieldClaimFailure) {
	...
	function claimSunshield(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsSunshieldClaimed[user] += bnbAmount;
		_totalBNBAsSunshieldClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $sunshield on a zero balance address:
		...
		// Depending on the % of $Sunshield tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsSunshield(address byAddress) public view returns (uint256) {
		return _bnbAsSunshieldClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsSunshield() public view returns (uint256) {
		return _totalBNBAsSunshieldClaimed;
	}
	...
	function reimburseAfterSunshieldClaimFailure() public view returns(bool) {
		return _reimburseAfterSunshieldClaimFailure;
	}


	function setReimburseAfterSunshieldClaimFailure(bool value) public onlyOwner {
		_reimburseAfterSunshieldClaimFailure = value;
	}
